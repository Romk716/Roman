...
R-00. Модуль должен позволять создать задачу с названием (строка) и описанием (строка).
R-01. При создании задачи модуль автоматически присваивает ей уникальный числовой идентификатор.
R-02. Модуль должен позволять получить список всех задач.
R-03. Модуль должен позволять получить задачу по её идентификатору.
R-04. Модуль должен позволять изменить статус задачи на один из допустимых: «новая», «в работе», «завершена».
R-05. Модуль не должен позволять изменить статус на недопустимое значение (должен возвращать ошибку или игнорировать).
R-06. Модуль должен позволять получить список задач, отфильтрованный по статусу.
R-07. Модуль должен позволять искать задачи по ключевому слову: если ключевое слово встречается в названии или описании (без учёта регистра), задача включается в результат.
R-08. При создании задачи модуль должен автоматически сохранять все задачи в JSON-файл.
R-09. При изменении статуса задачи модуль должен автоматически сохранять обновлённые данные в JSON-файл.
R-10. При запуске модуль должен загружать задачи из JSON-файла, если файл существует и корректен.
R-11. Если файл JSON отсутствует, модуль должен стартовать с пустым списком задач.
R-12. Если файл JSON повреждён, модуль должен стартовать с пустым списком задач и при сохранении перезаписать файл корректными данными.
R-13. Название задачи должно быть строкой длиной от 3 до 30 символов включительно. Допускаются буквы русского и латинского алфавита, цифры, пробелы и знаки препинания . , ! ? -.
R-14. Приоритет задаётся целым числом от 1 до 5 включительно. Значение должно быть целым.
R-15. Статус задачи может принимать одно из трёх значений: "new", "in_progress", "done". Регистр символов не учитывается (регистронезависимая проверка).
