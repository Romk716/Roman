...
R-01. Модуль должен позволять создать задачу с названием (строка) и описанием (строка).
R-02. При создании задачи модуль автоматически присваивает ей уникальный числовой идентификатор.
R-03. Модуль должен позволять получить список всех задач.
R-04. Модуль должен позволять получить задачу по её идентификатору.
R-05. Модуль должен позволять изменить статус задачи на один из допустимых: «новая», «в работе», «завершена».
R-06. Модуль не должен позволять изменить статус на недопустимое значение (должен возвращать ошибку или игнорировать).
R-07. Модуль должен позволять получить список задач, отфильтрованный по статусу.
R-08. Модуль должен позволять искать задачи по ключевому слову: если ключевое слово встречается в названии или описании (без учёта регистра), задача включается в результат.
R-09. При создании задачи модуль должен автоматически сохранять все задачи в JSON-файл.
R-10. При изменении статуса задачи модуль должен автоматически сохранять обновлённые данные в JSON-файл.
R-11. При запуске модуль должен загружать задачи из JSON-файла, если файл существует и корректен.
R-12. Если файл JSON отсутствует, модуль должен стартовать с пустым списком задач.
R-13. Если файл JSON повреждён, модуль должен стартовать с пустым списком задач и при сохранении перезаписать файл корректными данными.
